...
//   const rawPrice = cart?.[0]?.current_price;
//   console.log("priceeeeee is ", rawPrice);

//   // const price = cart.current_price;
...
export default function Cart1() {
  ...
  const updateCartQuantity = (id, newQuantity) => {
    if (newQuantity < 1 || newQuantity > 99) return;
    updateCart(id, newQuantity); // Make sure this updates your cart state properly
  };

  const subtotal = parseFloat(getTotalAmount()) || 0;
  const deliveryFee = parseFloat(setting?.shipping_cost) || 0;
  const grandtotalPrice = subtotal;
                ...
                >
                  {cart.length > 0 ? (
                    <table className="table table-container mb-3" id="table">
                      ...
                      <tbody>
                        {cart?.map((elm, i) => {
                          const parsedPrice = parseLocalizedPrice(
                            elm?.current_price
                          );
                          const productSubtotal = parsedPrice * elm?.quantity;
                          ...
                          return (
                            <tr key={i}>
                              ...
                              <td>
                                <span className="subtotal">
                                  {currency?.sign}
                                  {productSubtotal.toLocaleString("it-IT", {
                    ...
                    <p className="alert alert-warning m-3">
                      Your cart is empty!
                    </p>
                  )}
                </form>
                ...
                <table className="table table-borderless pb-3">
                  <tbody className="mb-3">
                    ...
                    <tr className="sub_total border_bottom">
                      ...
                      <td className="text_align_right">
                        {currency?.sign}
                        {(subtotal * 0.22).toLocaleString("it-IT", {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}
                      </td>
